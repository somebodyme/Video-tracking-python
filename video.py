...
# txt file za output
output = open("C:/Users/grego/OneDrive/Documents/FMF/Projektno delo/za analizo/pravokotni rotacija/izvoz.txt", "w+")
output.write("  Hitrost   Smer    Kotna hitrost \n")
i=1
while i<=5:
    #odpiranje zaporednih videov
    naslov = "C:/Users/grego/OneDrive/Documents/FMF/Projektno delo/za analizo/pravokotni rotacija/"+str(i)+".avi"
    print(naslov)

    film = cv2.VideoCapture(naslov)
    ...
    output.write(str(round(v_pred,2)) + "\t" + str(round(smer_pred,2)) + "\t" + str(round(kotna_hitrost_pred,2)) + "\t" + str(round(v_po,2)) + "\t" + str(round(smer_po,2)) + "\t" + str(round(kotna_hitrost_po,2)) + "\n")
    i +=1
